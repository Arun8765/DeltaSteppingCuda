...
__global__ void deltaSteppingKernel(
    ...
    // d_distParent[u].dist = 100;
    // d_distParent[u].parent = 0;
    return;
...
// __global__ void deltaStepper(
// // __device__ void deltaStepper(
//     int M,
//     int threadBlockSize,
//     int * d_rowPtr,
//     int * d_colInd,
...
void efficientDeltaSteppingCuda(vector<vector<vector<int>>> & matrix, const int source, vector<int> & distCuda){
    ...
    // deltaStepper<<<1, 1>>>(M, threadBlockSize, d_rowPtr, d_colInd, d_weights, d_distParent, delta, 0, stopperLength);
    // deltaStepper<<<1, 1>>>(M, threadBlockSize, d_rowPtr, d_colInd, d_weights, d_distParent, d_changeDone, delta, 0, stopperLength);
    ...
    //         break;
    //     }
    //     std::cout<<i<<" "<<distSeq[i]<<" "<<distCuda[i]<<endl;
    // }

    writePathToFile(cudaPath+to_string(source+1), cudaParent, source, distCuda);
    ...
    // std::cout<<"Writing to file doneasdjfajdj\n";


    // cout<<"Cuda Done. ajajajajajaj"<<endl;
...
void solve(int argc, char** argv){
    ...
    // efficientDeltaSteppingCuda(matrix, source, distCuda);
    // efficientDeltaSteppingCuda(matrix, source, distCuda, distSeq);
    // std::cout<<"Cuda Returned\n";
